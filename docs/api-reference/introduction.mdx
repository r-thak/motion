...
Motion computes optimized truck routes enriched with segment-level physics data — grade, curvature, fuel consumption, driver stress, and zone awareness. It exposes two interfaces to the same routing engine:

<CardGroup cols={2}>
  <Card title="Google-Compatible" icon="arrows-rotate" href="/api-reference/endpoint/google-compute-routes">
    Drop-in replacement for Google Routes API. Same request and response format — change only the base URL.
  </Card>
  <Card title="Route Resources" icon="route" href="/api-reference/endpoint/create-route">
...
| `PATCH` | `/v1/routes/{route_id}/driver-state` | Update driver state and re-score enrichment |
...
| `stressFactor` | Driver stress score based on maneuvers, grade, zones, and fatigue |
...
| Profile | Fuel | Stress | Grade | Zone |
|---------|------|--------|-------|------|
| `balanced` | 1.0 | 1.0 | 1.0 | 1.0 |
| `fuel_optimal` | 2.0 | 0.5 | 1.5 | 0.5 |
| `time_optimal` | 0.3 | 0.3 | 0.3 | 0.3 |
| `fatigue_aware` | 0.8 | 2.0 | 1.0 | 1.5 |
